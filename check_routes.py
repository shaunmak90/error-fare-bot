...
# Some APIs reject/reset connections that don't look like they're coming
# from a real client. Sending a normal User-Agent avoids that.
REQUEST_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (compatible; ErrorFareBot/1.0; "
        "+https://github.com/) requests-python"
    )
}


def request_with_retries(method: str, url: str, retries: int = 3, backoff: float = 2.0, **kwargs):
    """Wraps requests.get/post with retries for transient network failures
    (connection resets, timeouts, DNS blips) so one bad network moment
    ...
    print(f"Warning: request to {url} failed after {retries} attempts: {last_error}")
    return None
...
def get_baseline_price(tp_token: str, origin: str, destination: str) -> float | None:
    ...
    resp = request_with_retries("get", CALENDAR_URL, params=params)
...
def get_special_offers(tp_token: str, origin: str, destination: str) -> list[dict]:
    ...
    resp = request_with_retries("get", SPECIAL_OFFERS_URL, params=params)
    if resp is None or resp.status_code != 200:
        return []
...
def main() -> None:
    ...
    for route in routes:
        ...
        try:
            ...
            print(f"Warning: skipping {origin}->{destination} due to error: {e}")
            continue
        ...
        for offer in offers:
            ...
            try:
                ...
                print(f"Warning: failed to send Telegram alert for {uid}: {e}")
